...
export const resumir = (visitas: readonly Visita[], dias: readonly string[]): Resumen => {
  ...
  const navegadores = new Map<string, number>();
  ...
  for (const v of visitas) {
    ...
    if (v.navegador) sumar(navegadores, v.navegador);
    if (v.sistema) sumar(sistemas, v.sistema);
    if (v.idioma) sumar(idiomas, v.idioma);
  }
  ...
  return {
    ...
    navegadores: ordenar(navegadores),
    sistemas: ordenar(sistemas),
