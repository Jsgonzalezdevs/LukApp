import type { Category, TxKind } from '../types';

// Every key in this file MUST be pre-normalized (lowercase, no accents) or it
// can never match a token. `vocabulary.test.ts` asserts that, plus the critical
// invariant that no keyword collides with a numeral word — a keyword like `mil`
// would silently corrupt amounts.

/**
 * Direction phrases, longest sequence first. Order is load-bearing: `me costó`
 * must be tested before any rule that sees a leading `me` and assumes income,
 * because "me costó 80 mil" is an EXPENSE while "me pagaron 80 mil" is income.
 */
export const KIND_PHRASES: ReadonlyArray<{ seq: readonly string[]; kind: TxKind }> = [
  { seq: ['me', 'toco', 'pagar'], kind: 'gasto' },
  { seq: ['me', 'costo'], kind: 'gasto' },
  { seq: ['me', 'costaron'], kind: 'gasto' },
  { seq: ['me', 'valio'], kind: 'gasto' },
  { seq: ['me', 'valieron'], kind: 'gasto' },
  { seq: ['me', 'salio'], kind: 'gasto' },
  { seq: ['me', 'salieron'], kind: 'gasto' },
  { seq: ['me', 'cobraron'], kind: 'gasto' },
  { seq: ['me', 'descontaron'], kind: 'gasto' },
  { seq: ['me', 'entro'], kind: 'ingreso' },
  { seq: ['me', 'entraron'], kind: 'ingreso' },
  { seq: ['me', 'pago'], kind: 'ingreso' },
  { seq: ['me', 'pagaron'], kind: 'ingreso' },
  { seq: ['me', 'consignaron'], kind: 'ingreso' },
  { seq: ['me', 'llego'], kind: 'ingreso' },
  { seq: ['me', 'llegaron'], kind: 'ingreso' },
  { seq: ['me', 'transfirieron'], kind: 'ingreso' },
  { seq: ['me', 'devolvieron'], kind: 'ingreso' },
  { seq: ['me', 'giraron'], kind: 'ingreso' },
  { seq: ['me', 'abonaron'], kind: 'ingreso' },
  { seq: ['me', 'depositaron'], kind: 'ingreso' },
  { seq: ['me', 'prestaron'], kind: 'ingreso' },

  { seq: ['gaste'], kind: 'gasto' },
  { seq: ['pague'], kind: 'gasto' },
  { seq: ['compre'], kind: 'gasto' },
  { seq: ['merque'], kind: 'gasto' },
  { seq: ['saque'], kind: 'gasto' },
  { seq: ['retire'], kind: 'gasto' },
  { seq: ['inverti'], kind: 'gasto' },
  { seq: ['transferi'], kind: 'gasto' },
  { seq: ['cancele'], kind: 'gasto' },
  { seq: ['abone'], kind: 'gasto' },
  { seq: ['consigne'], kind: 'gasto' },
  { seq: ['recargue'], kind: 'gasto' },
  { seq: ['aporte'], kind: 'gasto' },
  { seq: ['tanquie'], kind: 'gasto' },
  { seq: ['tanquee'], kind: 'gasto' },
  { seq: ['tome'], kind: 'gasto' },
  { seq: ['comi'], kind: 'gasto' },
  { seq: ['pedi'], kind: 'gasto' },

  { seq: ['recibi'], kind: 'ingreso' },
  { seq: ['gane'], kind: 'ingreso' },
  { seq: ['cobre'], kind: 'ingreso' },
  { seq: ['vendi'], kind: 'ingreso' },
  { seq: ['consignaron'], kind: 'ingreso' },
  { seq: ['depositaron'], kind: 'ingreso' },
  { seq: ['transfirieron'], kind: 'ingreso' },
];

/** Every token that appears anywhere in KIND_PHRASES. */
export const KIND_WORDS: ReadonlySet<string> = new Set(KIND_PHRASES.flatMap((p) => p.seq));

/** Known Colombian brands and services. Checked before generic keywords. */
export const MERCHANTS: Record<string, Category> = {
  // Mercado
  exito: 'mercado',
  d1: 'mercado',
  ara: 'mercado',
  jumbo: 'mercado',
  olimpica: 'mercado',
  carulla: 'mercado',
  makro: 'mercado',
  zapatoca: 'mercado',
  colsubsidio: 'mercado',
  isimo: 'mercado',
  macro: 'mercado',
  alkosto: 'mercado',
  surtimax: 'mercado',
  merqueo: 'mercado',
  mercadolibre: 'otros',
  pricemart: 'mercado',
  megatiendas: 'mercado',
  supermercado: 'mercado',
  carniceria: 'mercado',
  fruver: 'mercado',
  plaza: 'mercado',
  panamericana: 'otros',
  oxxo: 'mercado',
  zenu: 'mercado',

  // Comida / Restaurantes
  rappi: 'comida',
  ifood: 'comida',
  didifood: 'comida',
  mcdonalds: 'comida',
  frisby: 'comida',
  kokoriko: 'comida',
  juanvaldez: 'comida',
  starbucks: 'comida',
  subway: 'comida',
  kfc: 'comida',
  burgerking: 'comida',
  elcorral: 'comida',
  crepes: 'comida',
  waffles: 'comida',
  crepesywaffles: 'comida',
  tostao: 'comida',
  oma: 'comida',
  pizzahut: 'comida',
  dominospizza: 'comida',
  dominos: 'comida',
  papajohns: 'comida',
  wok: 'comida',
  srwok: 'comida',
  jenos: 'comida',
  hornitos: 'comida',
  archies: 'comida',
  sierranevada: 'comida',
  andrescarne: 'comida',
  andrescarnederes: 'comida',
  ppc: 'comida',
  popsy: 'comida',
  mimos: 'comida',
  dunkin: 'comida',
  pbc: 'comida',
  bbc: 'entretenimiento',
  tada: 'entretenimiento',

  // Transporte y Vehículos
  transmilenio: 'transporte',
  sitp: 'transporte',
  metro: 'transporte',
  uber: 'transporte',
  didi: 'transporte',
  indriver: 'transporte',
  cabify: 'transporte',
  terpel: 'transporte',
  primax: 'transporte',
  texaco: 'transporte',
  esso: 'transporte',
  biomax: 'transporte',
  picap: 'transporte',
  tullave: 'transporte',
  civica: 'transporte',
  peaje: 'transporte',
  parqueadero: 'transporte',
  soat: 'transporte',
  tecno: 'transporte',
  mecanico: 'transporte',
  gasolina: 'transporte',
  pasajes: 'transporte',
  tiquetes: 'transporte',
  avianca: 'transporte',
  latam: 'transporte',
  wingo: 'transporte',

  // Transferencias / Finanzas
  nequi: 'transferencia',
  daviplata: 'transferencia',
  bancolombia: 'transferencia',
  davivienda: 'transferencia',
  bbva: 'transferencia',
  lulo: 'transferencia',
  lulobank: 'transferencia',
  bancodebogota: 'transferencia',
  colpatria: 'transferencia',
  itau: 'transferencia',
  rappipay: 'transferencia',
  cajasocial: 'transferencia',
  nubank: 'transferencia',
  nu: 'transferencia',
  dale: 'transferencia',
  uala: 'transferencia',
  cajero: 'transferencia',
  corresponsal: 'transferencia',
  cuota: 'otros',
  dmf: 'otros',
  rtf: 'otros',

  // Servicios y Hogar
  claro: 'servicios',
  movistar: 'servicios',
  tigo: 'servicios',
  etb: 'servicios',
  ptm: 'servicios',
  wom: 'servicios',
  directv: 'servicios',
  epm: 'servicios',
  codensa: 'servicios',
  vanti: 'servicios',
  enel: 'servicios',
  emcali: 'servicios',
  triplea: 'servicios',
  acueducto: 'servicios',
  gas: 'servicios',
  luz: 'servicios',
  agua: 'servicios',
  internet: 'servicios',
  admin: 'hogar',
  administracion: 'hogar',
  arriendo: 'servicios',
  homecenter: 'hogar',
  easy: 'hogar',
  chatgpt: 'servicios',
  openai: 'servicios',
  claude: 'servicios',
  anthropic: 'servicios',
  googleone: 'servicios',
  icloud: 'servicios',

  // Salud y Cuidado Personal
  farmatodo: 'salud',
  cruzverde: 'salud',
  locatel: 'salud',
  copidrogas: 'salud',
  larebaja: 'salud',
  pasteur: 'salud',
  cafam: 'salud',
  drogueria: 'salud',
  droguerias: 'salud',
  smartfit: 'salud',
  bodytech: 'salud',
  actionfitness: 'salud',
  sanitas: 'salud',
  sura: 'salud',
  compensar: 'salud',
  colsanitas: 'salud',
  eps: 'salud',
  prepagada: 'salud',
  medico: 'salud',
  odontologo: 'salud',
  barberia: 'salud',
  peluqueria: 'salud',
  unas: 'salud',

  // Entretenimiento, Ropa y Salidas
  netflix: 'entretenimiento',
  spotify: 'entretenimiento',
  hbo: 'entretenimiento',
  max: 'entretenimiento',
  disney: 'entretenimiento',
  paramount: 'entretenimiento',
  crunchyroll: 'entretenimiento',
  youtube: 'entretenimiento',
  steam: 'entretenimiento',
  apple: 'entretenimiento',
  playstore: 'entretenimiento',
  primevideo: 'entretenimiento',
  cinecolombia: 'entretenimiento',
  cinemark: 'entretenimiento',
  procinal: 'entretenimiento',
  zara: 'ropa',
  hmm: 'ropa',
  bershka: 'ropa',
  stradivarius: 'ropa',
  falabella: 'ropa',
  arturocalle: 'ropa',
  koaj: 'ropa',
  pullandbear: 'ropa',
  gef: 'ropa',
  puntohblanco: 'ropa',
  studiof: 'ropa',
  mattelsa: 'ropa',
  velez: 'ropa',
  bosi: 'ropa',
  nike: 'ropa',
  adidas: 'ropa',
  shein: 'ropa',
  decathlon: 'ropa',

  // Educación
  platzi: 'educacion',
  udemy: 'educacion',
  domestika: 'educacion',
  github: 'educacion',

  // Otras tiendas
  dollarcity: 'hogar',
  miniso: 'hogar',
  ikea: 'hogar',

  // Mascotas
  laika: 'hogar',
  agrocampo: 'hogar',
  veterinaria: 'hogar',
  petshop: 'hogar',
};

/** Restores accents and casing that dictation flattens. */
export const MERCHANT_DISPLAY: Record<string, string> = {
  exito: 'Éxito',
  d1: 'D1',
  ara: 'Ara',
  jumbo: 'Jumbo',
  olimpica: 'Olímpica',
  carulla: 'Carulla',
  makro: 'Makro',
  rappi: 'Rappi',
  mcdonalds: "McDonald's",
  frisby: 'Frisby',
  kokoriko: 'Kokoriko',
  juanvaldez: 'Juan Valdez',
  tostao: 'Tostao',
  oma: 'Oma',
  starbucks: 'Starbucks',
  subway: 'Subway',
  kfc: 'KFC',
  burgerking: 'Burger King',
  papajohns: "Papa John's",
  dominospizza: "Domino's Pizza",
  dominos: "Domino's Pizza",
  transmilenio: 'TransMilenio',
  sitp: 'SITP',
  metro: 'Metro',
  uber: 'Uber',
  didi: 'DiDi',
  indriver: 'inDriver',
  terpel: 'Terpel',
  primax: 'Primax',
  texaco: 'Texaco',
  biomax: 'Biomax',
  esso: 'Esso',
  picap: 'Picap',
  cabify: 'Cabify',
  nequi: 'Nequi',
  daviplata: 'Daviplata',
  bancolombia: 'Bancolombia',
  davivienda: 'Davivienda',
  nubank: 'NuBank',
  nu: 'Nu',
  lulobank: 'Lulo Bank',
  lulo: 'Lulo Bank',
  bbva: 'BBVA',
  bancodebogota: 'Banco de Bogotá',
  colpatria: 'Scotiabank Colpatria',
  itau: 'Itaú',
  uala: 'Ualá',
  rappipay: 'RappiPay',
  cajasocial: 'Banco Caja Social',
  claro: 'Claro',
  movistar: 'Movistar',
  tigo: 'Tigo',
  wom: 'WOM',
  etb: 'ETB',
  epm: 'EPM',
  enel: 'Enel',
  codensa: 'Codensa',
  vanti: 'Vanti',
  farmatodo: 'Farmatodo',
  cruzverde: 'Cruz Verde',
  locatel: 'Locatel',
  pasteur: 'Pasteur',
  larebaja: 'La Rebaja',
  netflix: 'Netflix',
  spotify: 'Spotify',
  udemy: 'Udemy',
  platzi: 'Platzi',
  homecenter: 'Homecenter',
  falabella: 'Falabella',
  zara: 'Zara',
  bershka: 'Bershka',
  stradivarius: 'Stradivarius',
  pullandbear: 'Pull&Bear',
  koaj: 'Koaj',
  arturocalle: 'Arturo Calle',
  elcorral: 'El Corral',
  crepesywaffles: 'Crepes & Waffles',
  crepes: 'Crepes & Waffles',
  smartfit: 'SmartFit',
  bodytech: 'Bodytech',
  cinecolombia: 'Cine Colombia',
  dollarcity: 'Dollarcity',
  miniso: 'Miniso',
  ikea: 'IKEA',
  laika: 'Laika',
  tullave: 'TuLlave',
  civica: 'Cívica',
  oxxo: 'Oxxo',
  didifood: 'DiDi Food',
  tada: 'TaDa',
  sierranevada: 'Sierra Nevada',
  andrescarne: 'Andrés Carne de Res',
  andrescarnederes: 'Andrés Carne de Res',
  ppc: 'PPC Pollo',
  popsy: 'Popsy',
  mimos: 'Mimo’s',
  dunkin: 'Dunkin’',
  chatgpt: 'ChatGPT',
  openai: 'OpenAI',
  claude: 'Claude AI',
  anthropic: 'Anthropic',
  googleone: 'Google One',
  icloud: 'iCloud',
  max: 'Max',
  paramount: 'Paramount+',
  crunchyroll: 'Crunchyroll',
  velez: 'Vélez',
  bosi: 'Bosi',
  studiof: 'Studio F',
  mattelsa: 'Mattelsa',
  nike: 'Nike',
  adidas: 'Adidas',
  shein: 'Shein',
  decathlon: 'Decathlon',
  github: 'GitHub',
  cafam: 'Cafam',
  colsanitas: 'Colsanitas',
};

const KEYWORDS_BY_CATEGORY: Record<Category, readonly string[]> = {
  mercado: [
    'mercado',
    'supermercado',
    'plaza',
    'verduras',
    'frutas',
    'carne',
    'huevos',
    'leche',
    'arroz',
    'granero',
    'tienda',
    'abarrotes',
    'despensa',
    'viveres',
    'carniceria',
    'fruver',
    'super',
    'panela',
    'papas',
    'pollo',
  ],
  comida: [
    'almuerzo',
    'desayuno',
    'cena',
    'comida',
    'restaurante',
    'domicilio',
    'domicilios',
    'pizza',
    'hamburguesa',
    'empanada',
    'empanadas',
    'cafe',
    'tinto',
    'helado',
    'pan',
    'panaderia',
    'corrientazo',
    'bandeja',
    'asado',
    'sushi',
    'onces',
    'algo',
    'asados',
    'asadero',
    'carnes',
    'malteada',
    'snack',
    'snacks',
    'postre',
    'dulces',
    'dulce',
    'pollo',
    'arepa',
    'arepas',
    'perrocaliente',
    'perroscalientes',
    'salchipapa',
    'cerveza',
    'cervezas',
    'pola',
    'polas',
    'trago',
    'tragos',
    'licor',
    'chuzo',
    'chuzos',
    'picada',
    'jugo',
    'gaseosa',
    'crepes',
    'waffles',
    'tacos',
    'fastfood',
    'comidita',
  ],
  transporte: [
    'gasolina',
    'transporte',
    'taxi',
    'bus',
    'pasaje',
    'pasajes',
    'peaje',
    'parqueadero',
    'parqueo',
    'moto',
    'carro',
    'avion',
    'vuelo',
    'tiquete',
    'tiquetes',
    'soat',
    'mecanico',
    'llantas',
    'lavado',
    'peajes',
    'uber',
    'didi',
    'cabify',
    'indriver',
    'picap',
    'transmilenio',
    'sitp',
    'metro',
    'tanqueo',
    'tanquear',
    'gas',
  ],
  servicios: [
    'luz',
    'agua',
    'internet',
    'celular',
    'factura',
    'recibo',
    'arriendo',
    'administracion',
    'energia',
    'telefono',
    'datos',
    'recarga',
    'servicios',
    'acueducto',
    'gas',
    'television',
    'cable',
    'wifi',
    'plan',
  ],
  salud: [
    'farmacia',
    'medicina',
    'medicamentos',
    'drogueria',
    'medico',
    'doctor',
    'eps',
    'odontologo',
    'dentista',
    'gym',
    'gimnasio',
    'examenes',
    'terapia',
    'vitaminas',
    'droga',
    'pastillas',
    'cita',
    'psicologo',
    'optica',
    'barberia',
    'peluqueria',
    'corte',
    'unas',
    'clinica',
  ],
  hogar: [
    'muebles',
    'ferreteria',
    'aseo',
    'detergente',
    'jabon',
    'escoba',
    'bombillo',
    'reparacion',
    'arreglo',
    'herramientas',
    'cocina',
    'colchon',
    'mascota',
    'perro',
    'gato',
    'veterinaria',
    'cuido',
    'purina',
    'alquiler',
    'casa',
    'limpieza',
  ],
  entretenimiento: [
    'cine',
    'bar',
    'cerveza',
    'cervezas',
    'salida',
    'fiesta',
    'concierto',
    'juego',
    'juegos',
    'videojuego',
    'suscripcion',
    'paseo',
    'discoteca',
    'trago',
    'tragos',
    'rumba',
    'entradas',
    'licor',
    'suscripciones',
    'apuestas',
    'pelicula',
    'boleta',
    'boletas',
  ],
  ropa: [
    'ropa',
    'camisa',
    'camiseta',
    'camisetas',
    'pantalon',
    'zapatos',
    'tenis',
    'chaqueta',
    'vestido',
    'gorra',
    'sudadera',
    'botas',
    'pantaloneta',
    'jeans',
    'jean',
    'prenda',
    'prendas',
  ],
  educacion: [
    'curso',
    'cursos',
    'libro',
    'libros',
    'universidad',
    'semestre',
    'matricula',
    'colegio',
    'cuaderno',
    'cuadernos',
    'utiles',
    'clase',
    'clases',
    'certificacion',
    'diplomado',
    'preuniversitario',
    'preicfes',
    'instituto',
    'academia',
    'pension',
    'estudio',
  ],
  transferencia: ['transferencia', 'giro', 'envio', 'retiro', 'cajero', 'consignacion', 'prestamo'],
  ahorro: ['ahorro', 'ahorros', 'cdt', 'inversion', 'fondo', 'fidu'],
  ingreso: [
    'salario',
    'sueldo',
    'nomina',
    'quincena',
    'bono',
    'prima',
    'cesantias',
    'venta',
    'ventas',
    'honorarios',
    'freelance',
    'propina',
    'comision',
    'subsidio',
    'reembolso',
    'ingreso',
  ],
  otros: [],
};

/** Flattened keyword -> category lookup, for O(1) token-equality matching. */
export const CATEGORY_KEYWORDS: Record<string, Category> = {};
for (const category of Object.keys(KEYWORDS_BY_CATEGORY) as Category[]) {
  for (const word of KEYWORDS_BY_CATEGORY[category]) {
    CATEGORY_KEYWORDS[word] = category;
  }
}

/**
 * Words that imply income on their own, with no direction verb present:
 * "el salario de este mes" needs no "me entró" to be understood.
 */
export const INCOME_IMPLIED: ReadonlySet<string> = new Set([
  'salario',
  'sueldo',
  'nomina',
  'quincena',
  'bono',
  'prima',
  'cesantias',
  'venta',
  'ventas',
  'honorarios',
  'freelance',
  'propina',
  'comision',
  'subsidio',
  'reembolso',
]);

/**
 * Nouns that make a preceding number a QUANTITY rather than an amount, so
 * "compré 2 pizzas por 30 mil" picks 30000 and not 2.
 */
export const COUNT_NOUNS: ReadonlySet<string> = new Set([
  'pizza',
  'pizzas',
  'unidad',
  'unidades',
  'persona',
  'personas',
  'hora',
  'horas',
  'dia',
  'dias',
  'semana',
  'semanas',
  'mes',
  'meses',
  'ano',
  'anos',
  'vez',
  'veces',
  'kilo',
  'kilos',
  'libra',
  'libras',
  'caja',
  'cajas',
  'cafe',
  'cafes',
  'tinto',
  'tintos',
  'pasaje',
  'pasajes',
  'litro',
  'litros',
  'galon',
  'galones',
  'paquete',
  'paquetes',
  'bolsa',
  'bolsas',
  'docena',
  'docenas',
  'pan',
  'panes',
  'empanada',
  'empanadas',
  'cerveza',
  'cervezas',
  'trago',
  'tragos',
  'entrada',
  'entradas',
  'tiquete',
  'tiquetes',
  'camiseta',
  'camisetas',
  'par',
  'pares',
  'almuerzo',
  'almuerzos',
  'desayuno',
  'desayunos',
  'cena',
  'cenas',
  'clase',
  'clases',
  'cuaderno',
  'cuadernos',
  'libro',
  'libros',
  'combo',
  'combos',
]);

/** A number right after one of these is very likely the amount. */
export const AMOUNT_CUES: ReadonlySet<string> = new Set([
  'por',
  'de',
  'en',
  'a',
  'vale',
  'valen',
  'valio',
  'valieron',
  'cuesta',
  'cuestan',
  'costo',
  'costaron',
  'salio',
  'salieron',
  'son',
  '$',
  'cuanto',
  'valor',
]);

/** Dropped when building the description. May overlap numeral words freely —
 *  this set is only consulted after the amount span has been removed. */
export const STOPWORDS: ReadonlySet<string> = new Set([
  'en',
  'de',
  'por',
  'el',
  'la',
  'los',
  'las',
  'un',
  'una',
  'unos',
  'unas',
  'del',
  'al',
  'pesos',
  'peso',
  'plata',
  'para',
  'con',
  'que',
  'hoy',
  'me',
  'se',
  'lo',
  'mi',
  'y',
  'a',
  'e',
  'o',
  'es',
  'fue',
  'este',
  'esta',
  'ya',
  'solo',
  'como',
  'mas',
]);

/** Payment methods the parser recognizes. */
export const PAYMENT_METHODS: Record<string, string> = {
  tarjeta: 'tarjeta_credito',
  tarjetacredito: 'tarjeta_credito',
  tarjetadebito: 'tarjeta_debito',
  credito: 'tarjeta_credito',
  debito: 'tarjeta_debito',
  mastercard: 'tarjeta_credito',
  visa: 'tarjeta_credito',
  master: 'tarjeta_credito',
  amex: 'tarjeta_credito',
  americanexpress: 'tarjeta_credito',
  nequi: 'billetera_digital',
  daviplata: 'billetera_digital',
  bancolombia: 'transferencia_bancaria',
  transferencia: 'transferencia_bancaria',
  transferi: 'transferencia_bancaria',
  efectivo: 'efectivo',
  cheque: 'cheque',
  bitcoin: 'cripto',
  crypto: 'cripto',
};
