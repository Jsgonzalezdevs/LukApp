...
describe('parseTransaction — description', () => {
  it('extracts concise and specific descriptions from conversational dictation', () => {
    expect(
      parseTransaction(
        'Compré con mi tarjeta de crédito unas banderitas que me costaron 16 mil pesos.',
      ).description,
    ).toBe('Banderitas');
    expect(parseTransaction('Me compré un café con mi tarjeta').description).toBe('Café');
    expect(
      parseTransaction('Me compré un detodito picante que me valió 2800 y').description,
    ).toBe('Detodito picante');
    expect(parseTransaction('gasté 20 mil en el mercado del D1').description).toBe(
      'Mercado del D1',
    );
    expect(
      parseTransaction('Me tomé un tintico en juan valdez por 4 lucas').description,
    ).toBe('Tintico en Juan Valdez');
    expect(
      parseTransaction(
        'Compré unos zapatos en falabella que me valieron 180 mil con la de crédito',
      ).description,
    ).toBe('Zapatos en Falabella');
    expect(parseTransaction('Pagué la factura de enel 85000').description).toBe(
      'Factura de Enel',
    );
    expect(
      parseTransaction('Me comí una hamburguesa doble con queso por 25 mil y ya').description,
    ).toBe('Hamburguesa doble con queso');
    expect(parseTransaction('Tanquié la moto en terpel con 20 mil').description).toBe(
      'Moto en Terpel',
    );
    expect(parseTransaction('Recargué la tarjeta tullave con 15 mil').description).toBe(
      'TuLlave',
    );
  });
