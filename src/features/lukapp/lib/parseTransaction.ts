...
/**
 * Limpia muletillas y frases conversacionales de dictado de voz para dejar
 * una descripción específica, limpia y concisa (ej. "Me compré un café con mi tarjeta" -> "Café",
 * "Compré con mi tarjeta de crédito unas banderitas que me costaron 16 mil" -> "Banderitas").
 */
export const limpiarDescripcionFinanciera = (desc: string): string => {
  if (!desc) return '';
  let t = desc.trim().replace(/^[,.\-•\s]+|[,.\-•\s]+$/g, '').trim();

  // 1. Corregir error común de transcripción "Me pagar" -> "Pago"
  t = t.replace(/^me\s+pagar\b/i, 'Pago');

  let prev = '';
  let iterations = 0;
  while (prev !== t && iterations < 6) {
    prev = t;
    iterations++;

    // Saludos, muletillas y frases introductorias
    t = t.replace(
      /^(?:hola\s+|buenas(?:\s+tardes|\s+dias|\s+días|\s+noches)?\s+|porfa(?:vor)?\s+|oye\s+|mira\s+|anota(?:me)?(?:\s+que)?\s+|registra(?:\s+que)?\s+|guarda(?:\s+que)?\s+|pon(?:me)?(?:\s+ahi|\s+ahí)?\s+|escribe(?:\s+que)?\s+)/i,
      '',
    );

    // Verbos de acción / compra / consumo al inicio
    t = t.replace(
      /^(?:me\s+)?(?:compre|compré|gaste|gasté|pague|pagué|adquiri|adquirí|pedi|pedí|anote|anoté|tome|tomé|comi|comí|tanquie|tanquié|tanquee|tanqueé|recargue|recargué|abone|aboné|cancele|cancelé|meti|metí|saque|saqué|puse)\s+(?:con\s+(?:(?:mi|la|el|una|un|mis)\s+)?(?:tarjeta(?:\s+de\s+(?:credito|crédito|debito|débito))?|de\s+(?:credito|crédito|debito|débito)|efectivo|nequi|daviplata|bancolombia|nu|nubank)\s+)?/i,
      '',
    );
    t = t.replace(/^(?:pago|compra|gasto)\s+de\s+/i, '');
    t = t.replace(/^(?:me\s+costo|me\s+costó|me\s+valio|me\s+valió|me\s+salio|me\s+salió)\s+/i, '');

    // Métodos de pago en cualquier parte
    t = t.replace(
      /\b(?:con\s+)?(?:(?:mi|la|el|mis)\s+)?(?:tarjeta(?:\s+de\s+(?:credito|crédito|debito|débito))?|de\s+(?:credito|crédito|debito|débito)|efectivo|plata\s+en\s+mano|transferencia|nequi|daviplata|bancolombia|nu|nubank|banco)\b/gi,
      '',
    );

    // Coletillas de costo/precio ("que me costaron", "que valieron", etc.)
    t = t.replace(
      /\s+(?:y\s+)?que\s+(?:me\s+)?(?:costaron|costo|costó|valieron|valio|valió|salieron|salio|salió)(?:\s+en)?(?:\s+y)?$/i,
      '',
    );
    t = t.replace(
      /\s+(?:que|y)\s+(?:me\s+)?(?:costaron|costo|costó|valieron|valio|valió|salieron|salio|salió)(?:\s+en)?\s*$/i,
      '',
    );
    t = t.replace(/\s+(?:por\s+valor\s+de|por|por\s+un\s+valor\s+de)$/i, '');

    // Muletillas de cierre de voz ('y ya', 'en total', 'y listo')
    t = t.replace(
      /\s+(?:y\s+ya|y\s+listo|eso\s+fue\s+todo|en\s+total|mas\s+o\s+menos|más\s+o\s+menos|aproximadamente)$/i,
      '',
    );

    // Coletillas de moneda / unidades sueltas al final
    ...
    t = t.replace(/^(?:el|la|los|las)\s+(?=factura|recibo|cuenta|arriendo|pago|cuota|tarjeta|moto|carro|auto|camioneta|mercado|almuerzo|desayuno|cena|gasolina)/i, '');

    // Preposiciones y conectores huérfanos al final o al inicio
    t = t.replace(/\s+(?:con|de|y|en|por|para|a|que)\s*$/i, '');
    t = t.replace(/^(?:con|de|y|en|por|para|a|que)\s+/i, '');
...
const detectAndConsumePaymentMethod = (
  ...
  for (let i = 0; i < tokens.length; i++) {
    ...
    if (method) {
      ...
      // Consumir preposiciones y artículos que preceden ("con mi", "en", "por la", "con la de")
      let p = i - 1;
      while (
        p >= 0 &&
        !consumed[p] &&
        ['con', 'en', 'por', 'usando', 'mi', 'la', 'el', 'una', 'un', 'mis', 'de', 'mediante'].includes(tokens[p].norm)
      ) {
        consumed[p] = true;
        p--;
      }

      // Consumir calificadores que siguen ("de credito", "de debito", "virtual", "fisica")
      let n = i + 1;
      if (n < tokens.length && !consumed[n] && tokens[n].norm === 'de') {
        if (
          n + 1 < tokens.length &&
          !consumed[n + 1] &&
          ['credito', 'debito', 'ahorros', 'corriente'].includes(tokens[n + 1].norm)
        ) {
          consumed[n] = true;
          consumed[n + 1] = true;
          n += 2;
        ...
        n < tokens.length &&
        !consumed[n] &&
        ['credito', 'debito', 'digital', 'virtual', 'fisica'].includes(tokens[n].norm)
      ) {
        consumed[n] = true;
        n++;
      }
...
export const parseTransaction = (
  ...
  if (isOCR) {
    ...
    const words: string[] = [];
    const stopWords = new Set(['en', 'el', 'la', 'de', 'del', 'a', 'los', 'las', 'un', 'una', 'con', 'por']);
    for (let k = 0; k < availTokens.length; k++) {
      const curr = availTokens[k].t;
      if (k + 1 < availTokens.length) {
        const next = availTokens[k + 1].t;
        if (!stopWords.has(curr.norm) || !stopWords.has(next.norm)) {
          const bigram = curr.norm + next.norm;
          if (MERCHANT_DISPLAY[bigram]) {
            words.push(MERCHANT_DISPLAY[bigram]);
            k++;
            continue;
          }
        }
      }
      words.push(MERCHANT_DISPLAY[curr.norm] ?? curr.raw);
    }
