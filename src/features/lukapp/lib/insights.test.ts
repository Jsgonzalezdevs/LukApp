...
describe('insightsDelMes — sí dice algo cuando hay patrón real', () => {
  ...
  it('detecta el gasto hormiga cuando de verdad se acumula y usa el nombre traducido de la categoría', () => {
    const txs = Array.from({ length: 8 }, () =>
      tx({ amountCop: 6_000, category: 'p-mssycx5flvcw' }),
    );
    const nombreDePersonalizado = (cat: string) => (cat === 'p-mssycx5flvcw' ? 'Snacks ricos' : cat);
    const r = insightsDelMes(txs, [], '2026-08', '2026-08-15', nombreDePersonalizado);
    const h = r.find((i) => i.id === 'gasto-hormiga');
    expect(h).toBeDefined();
    expect(h?.titulo).toContain('8 compras');
    expect(h?.detalle).toContain('Snacks ricos');
    expect(h?.detalle).not.toContain('p-mssycx5flvcw');
  });
  ...
  it('detecta tasa de ahorro alta con tono bien', () => {
    const txs = [
      tx({ kind: 'ingreso', amountCop: 1_000_000, occurredOn: '2026-08-01' }),
      tx({ kind: 'gasto', amountCop: 300_000, occurredOn: '2026-08-05' }),
    ];
    const r = insightsDelMes(txs, [], '2026-08', '2026-08-15', nombreDe);
    const a = r.find((i) => i.id === 'tasa-ahorro-alta');
    expect(a).toBeDefined();
    expect(a?.tono).toBe('bien');
    expect(a?.titulo).toContain('70%');
  });

  it('detecta gran gasto concentrado en el mes', () => {
    const txs = [
      tx({ id: 'laptop', kind: 'gasto', description: 'MacBook', amountCop: 500_000, occurredOn: '2026-08-05' }),
      tx({ kind: 'gasto', amountCop: 50_000, occurredOn: '2026-08-06' }),
      tx({ kind: 'gasto', amountCop: 50_000, occurredOn: '2026-08-07' }),
      tx({ kind: 'gasto', amountCop: 50_000, occurredOn: '2026-08-08' }),
    ];
    const r = insightsDelMes(txs, [], '2026-08', '2026-08-15', nombreDe);
    const g = r.find((i) => i.id.startsWith('gran-gasto'));
    expect(g).toBeDefined();
    expect(g?.titulo).toContain('MacBook');
  });

  it('nunca da más de tres, y el atento va primero', () => {
    // Fuerza varios insights a la vez.
