...
export const ForecastResumen: React.FC<Props> = React.memo(({ forecast }) => (
    ...
    {!forecast ? <p className="mt-3 text-[13px] text-[var(--fin-ink-faint)]">Forecast no disponible.</p> : (
      <div className="mt-3 grid grid-cols-1 gap-2 sm:grid-cols-3">
        {forecast.horizontes.map(({ dias, base, conservador, favorable }) => <article key={dias} className="rounded-[var(--fin-r-control)] bg-[var(--fin-bg)] p-3">
          ...
          <p className="mt-2 text-[12px] text-[var(--fin-ink-faint)]">Liquidez</p><p className="font-semibold tabular-nums text-[var(--fin-ink)]" aria-label={`Liquidez: ${valor(base.liquidezCop)}`}>{valor(base.liquidezCop)}</p>
          <p className="mt-1 text-[12px] text-[var(--fin-ink-faint)]">Disponible diario</p><p className="font-semibold tabular-nums text-[var(--fin-ink)]" aria-label={`Disponible diario: ${valor(base.disponibleDiarioCop)}`}>{valor(base.disponibleDiarioCop)}</p>
          <p className="mt-1 text-[12px] text-[var(--fin-ink-faint)]">Flujo neto</p><p className="font-semibold tabular-nums text-[var(--fin-ink)]" aria-label={`Flujo neto: ${valor(base.flujoNetoCop)}`}>{valor(base.flujoNetoCop)}</p>
          <p className="mt-2 text-[12px] leading-relaxed text-[var(--fin-ink-faint)]" aria-label={`Nivel de confianza: ${base.confianza}. Evidencia: ${base.evidencia.length ? base.evidencia[0] : 'sin evidencia adicional'}`}>
            Confianza: {base.confianza} · {base.evidencia.length ? base.evidencia[0] : 'Sin evidencia adicional'}
          </p>
