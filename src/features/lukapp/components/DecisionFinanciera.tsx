...
export const DecisionFinanciera: React.FC<Props> = ({ liquidez, entrada, movimientos, privacidad = false }) => {
  ...
  return <section className="mt-4 grid gap-3 sm:grid-cols-2" aria-label="Decisiones financieras">
    ...
    <div className="rounded-3xl border border-[var(--fin-line)] bg-[var(--fin-card)] p-4">
      ...
      <label className="mt-2 flex items-center gap-2 rounded-2xl border border-[var(--fin-line)] bg-[var(--fin-soft)] px-3 py-2 text-sm transition-colors focus-within:border-[var(--fin-accent)]"><span className="text-[var(--fin-ink-soft)]">$</span><input value={compra} onChange={(e) => setCompra(e.target.value.replace(/[^0-9]/g, ''))} inputMode="numeric" placeholder="Escribe un monto" className="!m-0 !w-full !border-0 !bg-transparent !p-0 text-[var(--fin-ink)] !outline-none !ring-0 placeholder:text-[var(--fin-ink-faint)] focus:!border-0 focus:!outline-none focus:!ring-0" aria-label="Monto de la compra" /></label>
